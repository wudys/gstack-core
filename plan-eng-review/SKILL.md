...
SLUG=$(~/.claude/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t docs/designs/*-$BRANCH-*.md docs/designs/*.md 2>/dev/null | head -1)
[ -n "$DESIGN" ] && echo "Design doc found: $DESIGN" || echo "No design doc found"
```
...
**STOP.** Do NOT proceed to Section 1 (Architecture review), edit the plan file with a proposed scope reduction, or end the review until the user responds. Naming the 80% solution in chat prose and continuing — or loading the AskUserQuestion schema via ToolSearch and then never invoking it — is the failure mode this gate exists to prevent.
...
**Anti-shortcut clause:** The plan file is the OUTPUT of the interactive review, not a substitute for it. Writing every finding into one plan write without walking the user through decisions is the failure mode to avoid. If you have ANY non-trivial finding in any review section, the path from finding to final artifact goes THROUGH AskUserQuestion. Zero findings in every section is the only path that bypasses AskUserQuestion. If you find yourself wanting to write a plan with findings before asking, stop and call AskUserQuestion now — that's the bug, recognize it.
...
**STOP.** Do NOT proceed to the next review section, edit the plan file with the proposed fix, or end the review until the user responds. An issue with an "obvious fix" is still an issue and still needs explicit user approval before it lands in the plan. Loading the AskUserQuestion schema via ToolSearch and then writing the recommendation as chat prose is the failure mode this gate exists to prevent.
...
Write to `docs/reviews/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
This command writes review metadata to `~/.gstack/` (user config directory,
not project files). The review dashboard depends on this data. Skipping this
command makes later review-readiness recommendations less accurate.
...
Always run this when a plan file is being reviewed. Write the review report to the plan file or to a repo-local docs/reviews/ artifact, depending on the user's preferred workflow.
...
Before ending the review, run this self-check. If any item fails, do the missing work:
...
4. If durable context will help later work, confirm the relevant docs/reviews/ artifact was written or updated.

Failing this gate is a contract violation —
